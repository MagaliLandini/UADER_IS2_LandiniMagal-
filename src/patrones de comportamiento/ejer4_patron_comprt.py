...
class AmState(State):
	...
	def toggle_am_memoria(self):
		print("Cambiando a Memoria")
		self.radio.state = self.radio.memoria
...
class FmState(State):
	...
	def toggle_fm_memoria(self):
		print("Cambiando a Memoria")
		self.radio.state = self.radio.memoria

#*------- Implementa como barrer las memorias
class Memoria(State):

	def __init__(self, radio):
		self.radio = radio
		self.stations = ["M1", "M2", "M3", "M4"]
		self.pos = 0
		self.name = "M"
...
#*--- Inicialmente en FM
	...
	def toggle_fm_memoria(self):
		self.state.toggle_fm_memoria()
		
	def toggle_am_memoria(self):
		self.state.toggle_am_memoria()
...
if __name__ == "__main__":
	...
	radio = Radio()
	actions = [radio.scan] * 3 + [radio.toggle_amfm] + [radio.scan] * 3 + [radio.toggle_am_memoria] + [radio.scan] * 4 + [radio.toggle_fm_memoria]
	actions *= 2
